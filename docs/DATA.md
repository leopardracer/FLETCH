...
- **Average holding period** — mean number of blocks from first buy to full exit, across fully closed positions. In blocks, not seconds (per-trade timestamps aren't recorded). With this, every wallet metric on `GET /api/wallets/:address` is either REAL or UNAVAILABLE with a reason — none are NOT_YET_IMPLEMENTED any more.
- **Linked wallets (clustering)** — `src/wallets/clusters.ts`, off `wallet_trades`, zero extra RPC. Another wallet is linked when both made their *first* buy of the same token within 2 blocks of each other on at least 2 different tokens. One shared token is noise; the same tight timing repeated across tokens is the pattern bundlers and sniper groups leave. Reported as a coordinated-entry timing pattern — explicitly not proof of common ownership. Funding-source tracing (who sent these wallets their ETH) needs native-transfer history FLETCH doesn't index and isn't claimed.
