...
- **Still honestly not there:** unrealized PnL (needs a live price per open position, including post-graduation v4 pricing), average holding period (only block numbers are stored per trade, not timestamps), early-entry timing aggregated across tokens. Each stays explicit `NOT_YET_IMPLEMENTED` on `GET /api/wallets/:address`. Post-graduation (Uniswap v4) trades aren't read, so a position exited after graduation shows as `OPEN` rather than as a guessed exit. The FLETCH Score's Smart Money component (`src/wallets/smartMoney.ts`) stays `unavailable` until enough real closed positions have accumulated across wallets to rank them — one wallet's win rate on two trades isn't a "smart money" label.
